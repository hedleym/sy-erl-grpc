...
-spec new(Connection::grpc_client_connection:connection(),
          Service::atom(),
          Rpc::atom(),
          Encoder::module(),
...
-spec handle_call(term(), term(), stream()) -> {reply, any(), any()} | {stop, normal, ok, any()} | {noreply, any(), non_neg_integer()}.
%% @private
...
handle_call({send_last, Message}, _From, Stream) ->
    {Res, Stream2} = send_msg(Stream, Message, true),
    {reply, Res, Stream2};
handle_call({send, Message}, _From, Stream) ->
    {Res, Stream2} = send_msg(Stream, Message, false),
    {reply, Res, Stream2};
